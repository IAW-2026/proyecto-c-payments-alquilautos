  ...
  return (
    <div className="bg-surface border border-border rounded-2xl shadow-card overflow-hidden flex-1">
      <div className="flex items-center justify-between py-5 px-7 border-b border-border-light flex-wrap gap-4 max-md:flex-col max-md:items-stretch max-md:p-4 max-md:gap-3 max-sm:p-2">
        <span className="font-semibold max-sm:text-[0.85rem]">Transacciones</span>
        <div className="flex gap-3 max-md:flex-wrap max-md:gap-2">
          <div className="flex items-center gap-2 border border-border rounded-lg py-[0.4rem] px-3 bg-white max-sm:py-1 max-sm:px-[0.4rem]">
            <input 
              type="date" 
              value={selectedDate}
              onChange={(e) => onDateChange(e.target.value)}
              className="border-none outline-none text-[0.8125rem] cursor-pointer max-sm:text-[0.7rem] max-sm:max-w-[100px]"
            />
            {selectedDate && (
          ...
          <thead>
            <tr className="bg-[#f8fafc]">
              {["ID", "Reserva", "Propietario", "Fecha", "Monto", "Estado"].map((h) => (
                <th key={h} className="py-3 px-7 text-center text-[0.72rem] font-semibold uppercase text-text-muted border-b border-border-light sticky top-0 z-10 bg-[#f8fafc] max-md:py-2 max-md:px-4 max-md:text-[0.65rem] max-sm:py-[0.3rem] max-sm:px-[0.4rem] max-sm:text-[0.55rem] max-sm:tracking-wider">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {transactions.length > 0 ? (
              transactions.map((t) => (
                <tr key={t.id} className="border-b border-border-light">
                  <td className="py-4 px-7 font-mono text-[0.8rem] text-center max-md:py-3 max-md:px-4 max-sm:py-[0.4rem] max-sm:px-[0.4rem] max-sm:text-[0.65rem]">{t.id}</td>
                  <td className="py-4 px-7 text-center max-md:py-3 max-md:px-4 max-sm:py-[0.4rem] max-sm:px-[0.4rem] max-sm:text-[0.7rem]">
                    <div className="flex items-center justify-center gap-[0.625rem] max-sm:gap-[0.375rem]">
                      ...
                      <span className="text-[0.875rem] font-medium max-sm:text-[0.7rem]">{t.cliente}</span>
                    </div>
                  </td>
                  <td className="py-4 px-7 text-[0.875rem] text-center max-md:py-3 max-md:px-4 max-sm:py-[0.4rem] max-sm:px-[0.4rem] max-sm:text-[0.7rem]">{t.vehiculo}</td>
                  ...
                  <td className="py-4 px-7 text-[0.875rem] font-semibold text-center max-md:py-3 max-md:px-4 max-sm:py-[0.4rem] max-sm:px-[0.4rem] max-sm:text-[0.7rem]">{formatCurrency(t.monto)}</td>
